...
class QQWorld_auto_save_images {
	...
	function __construct() {
		...
		switch ($this->mode) {
			case 'auto':
				add_action( 'wp_insert_post_data', array($this, 'fetch_images_when_saving'), 10, 2);
				break;
			case 'manual':
...
<?php
	...
	*/
	...
	function add_actions() {
		add_action( 'wp_insert_post_data', array($this, 'fetch_images_when_saving'), 10, 3);
	}

	function remove_actions() {
		remove_action( 'wp_insert_post_data', array($this, 'fetch_images_when_saving'), 10 );
	}
	...
	public function fetch_images_when_saving($data, $postarr) {
		...
		if ($allow) {
			...
			$data['post_content'] = $this->content_save_pre(stripslashes($data['post_content']), $postarr['ID']);
		}
		return $data;
	}
