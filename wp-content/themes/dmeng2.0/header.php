...
<meta property="qc:admins" content="35713601456206461667" />
...
if(!is_user_logged_in()){
	$script[] = sprintf("var loginUrl = '%s'", wp_login_url(dmeng_get_current_page_url()));
}
$script[] = sprintf("var dmengPath = '%s/'", get_bloginfo('template_url') );
if( !is_admin() && !is_preview() ) $script[] =  sprintf("var dmengTracker = %s", json_encode(dmeng_tracker_param()) );

echo join(';', $script).";"
?></script><script src="<?php echo get_bloginfo('template_url').'/js/bootstrap.min.js';?>"></script><script src="<?php echo get_bloginfo('template_url').'/js/dmeng-2.0.7.js';?>"></script>
</head>
<body <?php body_class(); ?>>
