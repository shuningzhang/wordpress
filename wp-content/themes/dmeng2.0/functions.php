...
function load_single_template($template) {
	...
	if( is_single() ) {
	  global $post;
	  // 判断所属分类别名中是否有-jc，有则代表是教程类分类
	  foreach((get_the_category()) as $category){
	      if ( strstr($category->category_nicename, "-jc") )
		  {
		      $new_template = locate_template(array('single-html.php' ));
		  }
	  }	  
	}
...
$dmeng_css_path = get_bloginfo('template_url').'/css/dmeng-2.0.7.css';
...
require_once( get_template_directory() . '/inc/latest.php' );
